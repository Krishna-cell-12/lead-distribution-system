...
# Find the very first export comment marker and cut there
# Keep only the clean engine body (before any export_report function)
marker = '\n// ----------------------------------------------------------------\n//  EXPORT HTML REPORT'
...
if idx != -1:
    base = content[:idx]
    print(f"Cut at char {idx}. Base length: {len(base)}")
else:
    base = content
    print("No marker found - using full content")

# The single, definitive, clean export_report function
export = '''
...
function export_report()
    ...
    html = '<!DOCTYPE html><html lang=''en''><head><meta charset=''UTF-8''>' + ..
        ...
        '75-100=Very Poor | >100=Unsafe. Score 100 = exactly at safe limit.' + ..
        '</div>' + ..
    ...
    // Use ascii(34) for double-quote char to avoid Scilab parser issues
    q   = ascii(34);
    ...
    disp('If browser did not open, copy the path above and open manually.');
    disp('Then press Ctrl+P -> Save as PDF.');

    messagebox('Report saved and browser opening!' + ascii(10) + ..
               report_file + ascii(10) + ..
               'Press Ctrl+P in browser -> Save as PDF', ..
               'AquaSafe Export', 'info');
endfunction
'''
...
# Count export_report occurrences
exp_count = sum(1 for l in lines if 'function export_report' in l)
print(f"export_report function count: {exp_count} (should be 1)")

# Check for double quotes outside comments in export section
in_exp = False
dq_issues = []
for i, l in enumerate(lines):
    ...
    if in_exp and '"' in l:
        ...
        if not stripped.startswith('//'):
            dq_issues.append((i+1, l.rstrip()))
if not dq_issues:
    print("PERFECT: Zero double-quotes in export_report (outside comments)")
else:
    print(f"Double-quote lines (outside comments): {len(dq_issues)}")
    for ln, txt in dq_issues:
        print(f"  Line {ln}: {txt}")
