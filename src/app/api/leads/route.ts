  ...
  try {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        // If it's a known error (logic error), don't retry
        if (error.code === 'DUPLICATE_LEAD' || error.code === 'SERVICE_NOT_FOUND' || error.code === 'INSUFFICIENT_QUOTA') {
          throw error;
        }

        // If it's a Prisma deadlock or conflict (P2034, P2002 on assignments etc), retry
        attempt++;
        if (attempt < maxRetries) {
          // Exponential backoff: 2^attempt * 100ms + random jitter up to 500ms
          const delay = (Math.pow(2, attempt) * 100) + Math.floor(Math.random() * 500);
          console.warn(`Transaction conflict on attempt ${attempt}. Retrying in ${Math.round(delay)}ms...`);
